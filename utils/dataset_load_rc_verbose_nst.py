...
def processor_normal_no(sample):
    ...
    if sample["source"] == "nst" and sample["task"] == "transcribe" and sample and sample["language"] == "no":
        return sample

def processor_normal_nn(sample):
    sample = processor_normal(sample)
    if sample["source"] == "nst" and sample["task"] == "transcribe" and sample and sample["language"] == "nn":
        return sample
...
def processor_previous_text_prompts(sample):
    if sample["source"] == "nst" and sample["task"] == "transcribe" and sample["previous_text"] is not None:
...
def load_dataset_nbwhisper_verbose_nst(dataset_name, dataset_config_name=None, split="train", streaming=True, **kwargs):
    if split == "train":
        processors = [
            processor_normal_no,
            processor_normal_nn,
