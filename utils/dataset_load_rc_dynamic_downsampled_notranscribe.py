...
def downsample_wrapper(processor_function):
    """
    Wrapper function that applies downsampling to samples with task 'transcribe' after the primary processor function.
    """
    def wrapped_processor(sample):
        processed_sample = processor_function(sample)
        if processed_sample and processed_sample.get("task") == "transcribe":
            return None
