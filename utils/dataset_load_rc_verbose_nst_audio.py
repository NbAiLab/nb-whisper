...
def processor_normal_no(sample):
    ...
    if (sample["source"] == "audio_books" or sample["source"] == "nst") and sample["task"] == "transcribe" and sample and sample["language"] == "no":
        return sample

def processor_normal_nn(sample):
    sample = processor_normal(sample)
    if (sample["source"] == "audio_books" or sample["source"] == "nst") and sample["task"] == "transcribe" and sample and sample["language"] == "nn":
        return sample

def processor_normal_en(sample):
    sample = processor_normal(sample)
    if sample["task"] == "transcribe" and sample and sample["text_en"] is not None and sample["source"] != "audio_books":
        return {
            **sample,
            "text": sample["text_en"],
            "language": "en",
...
def processor_timestamps(sample):
    if sample["task"] == "transcribe" and sample["timestamped_text"] is not None:
        return {**sample, "text": sample["timestamped_text"], "previous_text": None}

def processor_timestamps_en(sample):
    if sample["task"] == "transcribe" and sample["timestamped_text_en"] is not None and sample["source"] != "audio_books":
...
def processor_previous_text_prompts(sample):
    if (sample["source"] == "audio_books" or sample["source"] == "nst") and sample["task"] == "transcribe" and sample["previous_text"] is not None:
...
def load_dataset_nbwhisper_verbose_nst_audio(dataset_name, dataset_config_name=None, split="train", streaming=True, **kwargs):
    if split == "train":
        processors = [
            processor_normal_no,
            processor_normal_nn,
