...
def filtered_wrapper(processor_function):
    """
    Wrapper function that only returns samples where the 'id' starts with "SA".
    """
    def wrapped_processor(sample):
        # Process the sample using the provided processor function
        processed_sample = processor_function(sample)
        if processed_sample is None:
            return None

        # Check if the 'id' starts with "SA"
        if processed_sample.get("id", "").startswith("SA"):
