...
def filtered_wrapper(processor_function):
    ...
    def wrapped_processor(sample):
        ...
        if processed_sample.get("task") == "translate":
            ...
            # Check for duplicate words; if present, change task to 'transcribe'
            if (text and is_duplicate_word_present(text)) or (timestamped_text and is_duplicate_word_present(timestamped_text)):
                processed_sample["task"] = "transcribe"
                return processed_sample

            # Check for presence of "…" in text and timestamped_text
...
def processor_normal_no(sample):
    ...
    if sample and sample["language"] == "no" and sample["source"] != "nrk_tv_silence":
        return sample

def processor_normal_nn(sample):
    sample = processor_normal(sample)
    if sample and sample["language"] == "nn" and sample["source"] != "nrk_tv_silence":
        return sample

def processor_normal_en(sample):
    sample = processor_normal(sample)
    if sample and sample["text_en"] is not None and sample["source"] != "audio_books" and sample["source"] != "nrk_tv_silence":
        return {
            **sample,
            "text": sample["text_en"],
            "language": "en",
...
def processor_timestamps(sample):
    if sample["timestamped_text"] is not None and sample["source"] != "nrk_tv_silence":
        return {**sample, "text": sample["timestamped_text"], "previous_text": None}

def processor_timestamps_en(sample):
    if sample["timestamped_text_en"] is not None and sample["source"] != "audio_books"  and sample["source"] != "nrk_tv_silence":
...
def processor_previous_text_prompts(sample):
    if sample["previous_text"] is not None  and sample["source"] != "nrk_tv_silence":
        return {**sample, "timestamped_text": None}

def processor_timestamps_previous_text(sample):
    if sample["previous_text"] is not None and sample["timestamped_text"] is not None and sample["source"] != "nrk_tv_silence":
